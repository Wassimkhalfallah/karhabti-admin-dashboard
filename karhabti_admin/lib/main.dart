  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      routes: {
        ...
        '/responsable': (_) => const ResponsableDashboardScreen(),
...
class _RoleRouterScreenState extends State<RoleRouterScreen> {
  ...
  Future<void> _redirect() async {
    ...
    if (role == UserRole.responsableTechnicien) {
      final user = _auth.currentUser;
      final hasGarage = user != null && await _responsableService.hasGarage(user.id);
      Navigator.of(context).pushReplacementNamed(hasGarage ? '/responsable' : '/garage-setup');
      return;
    }
  ...
  static final List<_NavSection> _sections = [
    _NavSection(
      ...
      items: [
        ...
        _NavItem(label: 'Responsables',       icon: Icons.engineering_rounded,          index: 5),
      ],
    ),
    _NavSection(
      label: 'Garages PRO',
      ...
      accentColor: _T.gold,
      items: [
        _NavItem(label: 'Garages (lecture)', icon: Icons.store_rounded,        index: 10, badge: 'PRO', badgeColor: _T.gold),
      ],
    ),
    _NavSection(
      ...
      items: [
        _NavItem(label: 'Feedbacks',   icon: Icons.feedback_rounded,  index: 6, badge: '4', badgeColor: _T.danger),
        _NavItem(label: 'Analytiques', icon: Icons.analytics_rounded, index: 7),
      ],
    ),
    _NavSection(
      label: 'Paramètres',
      ...
      items: [
        _NavItem(label: 'Paramètres', icon: Icons.settings_outlined,      index: 8),
        _NavItem(label: 'Profil',     icon: Icons.person_outline_rounded,  index: 9),
      ],
    ),
  ];
